...
in more values from the sequence. From the question statement:
...
The median is maintained using two heaps, a min heap (root is smallest element) and a max heap (root is largest element). The smallest n/2 elements
are stored in the max heap while the largest n/2 elements are stored in the min heap (minHeap is "smallest of the maxes", maxHeap is "king of the mins").

By maintaining this invariant, we can be assured the the roots of the
...
of the two heaps are with +-1 of each other. If they aren't, pop root of whichever tree has more and insert it into the tree with less.
